...
def on_connect(client, userdata, flags, rc, v5config=None):
    print(dt.now().strftime("%H:%M:%S.%f")[:-2] + " Connection returned result: "+ack(rc) + " my userdata is " + str(userdata))

def on_message(client, userdata, message,tmp=None):
    print(dt.now().strftime("%H:%M:%S.%f")[:-2] + " Received message " + str(message.payload) + " on topic '"
        + message.topic + "' with QoS " + str(message.qos) + " my userdata is " + str(userdata))

def on_publish(client, userdata, mid,tmp=None):
    print(dt.now().strftime("%H:%M:%S.%f")[:-2] + " Published message id: "+str(mid) + " my userdata is " + str(userdata))
...
def on_subscribe(client, userdata, mid, qos,tmp=None):
    ...
    print(dt.now().strftime("%H:%M:%S.%f")[:-2] + " Subscribed " + qos_msg + " my userdata is " + str(userdata))    